...
func _process(delta):
	...
	if current_state == 1 or current_state == 0:
		...
		$AnimatedSprite2D.play("yellow_idle")
	elif current_state == 2 and !chatting:
		if dir.x == -1:
			anim.flip_h = true
			$AnimatedSprite2D.play("yellow_walk_e")
		if dir.x == 1: 
			anim.flip_h = false
			$AnimatedSprite2D.play("yellow_walk_e")
		if dir.y == -1: 
			anim.flip_h = true
			$AnimatedSprite2D.play("yellow_walk_n")
		if dir.y == 1: 
			anim.flip_h = false
			$AnimatedSprite2D.play("yellow_walk_s")
